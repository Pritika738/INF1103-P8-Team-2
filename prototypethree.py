...
- No file operations here (the demo block's mock-file loaders - one
  for a fresh reading, one for history - are the one exception, and
  they are isolated inside __main__ too).
...
shaped like:
    {"heart_rate": number or None,
     "blood_pressure": <see below> or None,
     "blood_glucose": number or None,
     "date": string or None}

`date` is optional and carried through untouched: this module reads it
in flatten_ai_metrics() and returns it as a top-level "date" key from
process_ai_record(), purely so the Data Manager can persist it
alongside the record. Per this project's layered architecture, the
Logic Manager itself never writes files, formats, or validates dates -
that stays the Data Manager's job.

`blood_pressure` is tolerated in two shapes, since different AI
extraction passes may return either:
    - a dict:    {"systolic": number or None, "diastolic": number or None}
    - a string:  "125/67" (whitespace around either part is fine)
Anything else (missing key, None, an unparsable string, a string not
shaped like "X/Y") is treated as "no blood pressure reading" rather
than raising - this module never crashes the pipeline over a
...
# raw output may bundle it under one "blood_pressure" key - see
# flatten_ai_metrics() / _parse_blood_pressure() below.
...
# A single CURRENT reading at or below these values is the low-side
# mirror of URGENT_THRESHOLDS: also an immediate, high-severity
# concern regardless of trend history (e.g. systolic < 90 / diastolic
# < 60 mmHg is widely cited as hypotension; glucose < 3.9 mmol/L /
# ~70 mg/dL is a commonly cited hypoglycemia threshold; heart_rate
# < 50 bpm is bradycardia).
URGENT_LOW_THRESHOLDS = {
...
def _parse_blood_pressure(raw: Any) -> Tuple[Optional[float], Optional[float]]:
    """
    Normalize a `blood_pressure` value from ai_manager's output into a
    (systolic, diastolic) tuple, tolerating either shape it might
    arrive in:
        - a dict:   {"systolic": ..., "diastolic": ...}
        - a string: "125/67" (leading/trailing whitespace on the whole
          string, or around either part, is stripped before parsing)

    Anything else - raw is None, a string missing the "/" separator,
    a string with more than two parts, or parts that aren't valid
    numbers - is treated as "no reading available" and returns
    (None, None) rather than raising, so a single malformed upstream
    value never crashes the pipeline.

    Args:
        raw: the value of ai_extracted["blood_pressure"], in whatever
            shape the AI extraction step produced it.

    Returns:
        (systolic, diastolic) as numbers, or (None, None) if `raw`
        was missing, malformed, or unparsable.
    """
    if raw is None:
        return None, None

    if isinstance(raw, dict):
        return raw.get("systolic"), raw.get("diastolic")

    if isinstance(raw, str):
        parts = raw.strip().split("/")
        if len(parts) != 2:
            return None, None  # not shaped like "X/Y"

        try:
            systolic = float(parts[0].strip())
            diastolic = float(parts[1].strip())
        except (ValueError, TypeError):
            return None, None  # parts weren't valid numbers

        # Collapse whole-number floats (125.0) down to int (125) so
        # readings parsed from a string look identical to readings
        # that arrived pre-split as a dict of ints.
    ...
    # Some other type entirely (int, list, ...) - not a shape we
    # recognise, so treat as no reading rather than guessing.
    return None, None


def flatten_ai_metrics(ai_extracted: Optional[Dict[str, Any]]) -> Dict[str, Optional[float]]:
    """
    Convert ai_manager's extraction shape into the flat key names used
    everywhere else in this module (TRACKED_METRICS), which also match
    how data_manager stores historical records - so a fresh reading
    and past readings can be compared using identical keys.

    Blood pressure is normalized via _parse_blood_pressure(), so it is
    handled whether ai_extracted["blood_pressure"] arrived as a dict
    ({"systolic": ..., "diastolic": ...}) or as a single formatted
    string ("125/67").

    "date" is carried through as-is (whatever type/format ai_extracted
    used, typically a string) purely so downstream callers - namely
    the Data Manager - have it available when persisting the record.
    This module never parses, validates, or reformats it: doing so
    would step outside the Logic Manager's role as pure business
    rules, with no file/date-formatting concerns of its own.
    ...
    Returns:
        A flat dict with exactly the keys in TRACKED_METRICS, plus
        "date". Always returns all five keys - missing/unavailable
        ones are None - even if `ai_extracted` itself is None or
        malformed.
    """
    if not isinstance(ai_extracted, dict):
        return {**{metric: None for metric in TRACKED_METRICS}, "date": None}

    systolic, diastolic = _parse_blood_pressure(ai_extracted.get("blood_pressure"))
...
def evaluate_health_metrics(
    ...
    1. URGENT - a multi-condition-free but immediate check: does the
       CURRENT reading alone cross URGENT_THRESHOLDS (too high) or
       URGENT_LOW_THRESHOLDS (too low)? A single dangerously high OR
       dangerously low reading is worth flagging right away, without
    ...
    Args:
        ...
        current_date: today's record date (e.g. from
            flatten_ai_metrics()["date"]), used only to label a
            FLAGGED trend finding's "trend_dates" - never compared
            against or validated. Optional; a FLAGGED finding still
            fires without it, just with today's slot left as None.

    Returns:
        A list of finding dicts, each with "metric", "status" ("URGENT"
        or "FLAGGED"), and "reason". A "FLAGGED" finding also carries
        "trend_dates": the three visit dates behind the trend, oldest
        first - [two-visits-ago, last-visit, current_date] - each
        entry None if that record had no date on file. Never mentions
        a disease name or a treatment - only describes the numbers and
        the pattern.
    """
    findings = []
    ...
    for metric in TRACKED_METRICS:
        ...
        # Checked both directions - a critically HIGH reading and a
        # critically LOW reading are equally urgent and mutually
        # exclusive, so at most one of these two branches can fire.
        if current_val >= URGENT_THRESHOLDS[metric]:
            findings.append({
                "metric": label,
                "status": "URGENT",
                "reason": (
                    f"{label} reading of {current_val} is critically "
                    "elevated and usually warrants prompt medical attention."
                ),
            })
            continue  # already the most severe finding possible for this metric

        if current_val <= URGENT_LOW_THRESHOLDS[metric]:
            findings.append({
                "metric": label,
                "status": "URGENT",
                "reason": (
                    f"{label} reading of {current_val} is critically low "
                    "and usually warrants prompt medical attention."
                ),
            })
            continue  # already the most severe finding possible for this metric
        ...
        if past_1 is not None and past_2 is not None and current_val > past_1 > past_2:
            past_1_date = historical_records[-1].get("date")
            past_2_date = historical_records[-2].get("date")
            trend_dates = [past_2_date, past_1_date, current_date]
            ...
            if all(trend_dates):
                reason += f" ({past_2_date} \u2192 {past_1_date} \u2192 {current_date})."
            else:
                reason += "."
...
def analyze_recent_changes(
    ...
    Args:
        ...
        historical_records: list of past health record dicts, assumed
            sorted oldest to newest.
        current_date: today's record date (e.g. from
            flatten_ai_metrics()["date"]), used only to label each
            change's "current_date" field and feedback text - never
            compared against or validated. Optional; a change still
            fires without it, just with that field left as None.

    Returns:
        A list of change dicts, each with "metric", "status"
        ("CHANGED"), "feedback", "compared_to_date" (the prior
        record's date the comparison was made against, or None if
        that record had no date on file), and "current_date" (the
        value passed in above, carried through unchanged).
    """
    changes = []
    ...
    compared_to_date = last_record.get("date")
    ...
    for metric in TRACKED_METRICS:
        ...
        # Prefer naming the actual prior date; fall back to the old
        # generic phrasing when it's missing so the sentence never
        # reads "since your last record on None".
        if compared_to_date:
            since_phrase = f"since your last record on {compared_to_date}"
        else:
            since_phrase = "since your last record"
        if current_date:
            since_phrase += f" (today: {current_date})"
...
def process_ai_record(
    ...
    Returns:
        A dict with "metrics", "date" (carried through unchanged from
        ai_extracted, for the Data Manager to persist - this module
        never reads, writes, or validates it further), "urgent_findings",
        "flagged_trends", "recent_changes", "risk_score", "decision"
        (one of "REJECTED", "URGENT", "FLAGGED", "ACCEPTED"),
        "recommended_action", "requires_doctor_review", and "summary".
    """
    flattened = flatten_ai_metrics(ai_extracted)
    record_date = flattened.get("date")
    # "metrics" stays limited to the four numeric readings - "date" is
    # surfaced as its own top-level key instead, so a consumer reading
    # "metrics" never has to special-case a non-numeric entry in it.
    current_metrics = {metric: flattened.get(metric) for metric in TRACKED_METRICS}
    ...
    findings = evaluate_health_metrics(current_metrics, historical_records, current_date=record_date)
    urgent_findings = [f for f in findings if f["status"] == "URGENT"]
    flagged_trends = [f for f in findings if f["status"] == "FLAGGED"]
    recent_changes = analyze_recent_changes(current_metrics, historical_records, current_date=record_date)
...
if __name__ == "__main__":
    ...
    # Resolve the mock file relative to THIS script's own directory
    # (not the current working directory), so `python logic_manager.py`
    ...
    try:
        with open(MOCK_AI_OUTPUT_PATH, "r", encoding="utf-8") as mock_file:
            mock_ai_extracted = json.load(mock_file)
    except FileNotFoundError:
        print(
            f"[demo] Mock file not found at {MOCK_AI_OUTPUT_PATH}; "
            "proceeding with ai_extracted=None (simulates the AI Manager "
            "giving up after retrying)."
        )
    except json.JSONDecodeError as exc:
        print(
            f"[demo] Mock file at {MOCK_AI_OUTPUT_PATH} is not valid JSON "
            f"({exc}); proceeding with ai_extracted=None."
        )
    ...
    # Loaded the same way as the fresh reading above - resolved
    ...
    mock_history = []
    try:
        with open(MOCK_HISTORY_PATH, "r", encoding="utf-8") as history_file:
            mock_history = json.load(history_file)
    except FileNotFoundError:
        print(
            f"[demo] Mock file not found at {MOCK_HISTORY_PATH}; "
            "proceeding with historical_records=[] (simulates a first-ever "
            "record for a new user with no history yet)."
        )
    except json.JSONDecodeError as exc:
        print(
            f"[demo] Mock file at {MOCK_HISTORY_PATH} is not valid JSON "
            f"({exc}); proceeding with historical_records=[]."
        )

    print(
        f"--- Simulating a record loaded from {MOCK_AI_OUTPUT_PATH}, "
        f"history from {MOCK_HISTORY_PATH} ---"
    )
